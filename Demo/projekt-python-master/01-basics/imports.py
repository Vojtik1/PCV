...
for i in range(years_to_check):
    ...
    print(f"Velikonoční neděle v roce {year} bude {easter_date.strftime('%d.%m.%Y')}")
...
while True:
    christmas_date = datetime.date(current_year, 12, 24)
    if christmas_date.weekday() == 6:
        break
    current_year += 1
